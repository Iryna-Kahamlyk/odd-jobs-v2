// merged script.js — consolidated, guarded, and compatible with both ID variants
(function () {
    ...
    function init() {
      ...
      const jobFilter = document.getElementById('jobFilter') || document.getElementById('ordersScope'); // dropdown
      const ongoingTab = document.getElementById('ongoingTab') || document.getElementById('tabOngoing') || document.getElementById('tabOngoing'); // tab (several fallback ids)
      const previousTab = document.getElementById('previousTab') || document.getElementById('previousTab') || document.getElementById('tabAccepted');
  
      // older code used tabOngoing/tabAccepted/tab names; map them:
      const tabOngoing = document.getElementById('tabOngoing') || document.getElementById('ongoingTab');
      const tabAccepted = document.getElementById('tabAccepted') || document.getElementById('previousTab');
      ...
      function id() { return 'job_' + Date.now() + '_' + Math.random().toString(36).slice(2, 6); }
      ...
      function seedDemoIfNeeded() {
        ...
        const now = Date.now();
        const demo = [
          { title: 'Cat sitter', description: 'Look after my cat for the evening', price: 15, location: 'Home', category: 'chores', status: 'accepted', claimer: { name: 'Jason' }, created: now - 1000*60*60*24*5 },
          { title: 'Bring boxes to car', description: 'Move boxes from HA to Nest parking', price: 5, location: 'HA', category: 'chores', status: 'accepted', claimer: { name: 'Pantos' }, created: now - 1000*60*60*24*4 },
          { title: 'Formatting citations', description: 'Teach me to format APA', price: 12, location: 'Library', category: 'educational', status: 'accepted', claimer: { name: 'Keeb' }, created: now - 1000*60*60*24*3 },
          { title: 'Setup printer', description: 'Help me set up my new printer', price: 8, location: 'Home', category: 'chores', status: 'accepted', claimer: { name: 'Melissa' }, created: now - 1000*60*60*24*2 },
          { title: 'Grocery run', description: 'Buy milk & eggs', price: 10, location: 'Local store', category: 'groceries', status: 'accepted', claimer: { name: 'Liam' }, created: now - 1000*60*60*12 },
          { title: 'Math tutoring', description: '1 hour calculus help', price: 20, location: 'Cafe', category: 'educational', status: 'accepted', claimer: { name: 'Ava' }, created: now - 1000*60*60*6 },
        ];
        jobs = demo.map(j => ({
          id: id(),
          title: j.title,
      ...
      if (logoutBtn) {
        logoutBtn.addEventListener('click', () => {
          // customizable — for now just clear demo user and show find
      ...
      // --- Find: open jobs + apply ---
      function renderFindGrid() {
        if (!findGrid) return;
        findGrid.innerHTML = '';
        const open = jobs.filter(j => j.status === 'open').sort((a, b) => b.created - a.created);
  
        if (statOpenCount) statOpenCount.textContent = open.length;
  
        if (!open.length) {
          findGrid.innerHTML = '<div class="empty">No open jobs. Post one or check back later.</div>';
          return;
        }
  
        for (const job of open) {
          const tile = document.createElement('div'); tile.className = 'job-tile ongoing';
  
          const rowTop = document.createElement('div'); rowTop.className = 'row-top';
          const imgDiv = document.createElement('div'); imgDiv.className = 'img'; imgDiv.textContent = 'image';
          const content = document.createElement('div'); content.className = 'content';
          const title = document.createElement('div'); title.className = 'title'; title.textContent = job.title;
          const subtitle = document.createElement('div'); subtitle.className = 'subtitle'; subtitle.textContent = `${job.category} • ${new Date(job.created).toLocaleDateString()}`;
          const desc = document.createElement('div'); desc.className = 'desc'; desc.textContent = job.description;
  
          content.appendChild(title); content.appendChild(subtitle); content.appendChild(desc);
          rowTop.appendChild(imgDiv); rowTop.appendChild(content);
          tile.appendChild(rowTop);
  
          const actions = document.createElement('div'); actions.className = 'row'; actions.style.marginTop = '10px';
          const applyBtn = document.createElement('button'); applyBtn.className = 'btn'; applyBtn.type = 'button'; applyBtn.textContent = 'Apply now';
          applyBtn.onclick = () => applyToJob(job.id);
          actions.appendChild(applyBtn);
  
          tile.appendChild(actions);
          findGrid.appendChild(tile);
        }
      }
      ...
      function applyToJob(jobId) {
        ...
        // Demo behavior:
        // if name === "Guest" (case-insensitive) -> auto-accept for you (set current user)
        // otherwise -> leave as applicant (poster can accept later) — earlier requests had variants where non-Guest accepted for others; keep applicant model
      ...
      // --- Post view: active listings + waitlist preview (basic) ---
      function renderActiveListings() {
        if (!activeListingsEl) return;
        activeListingsEl.innerHTML = '';
        const mine = jobs.filter(j => j.postedBy === 'me' && j.status === 'open').sort((a, b) => b.created - a.created);
        ...
        for (const job of mine) {
          ...
          const subh = document.createElement('div'); subh.className = 'small'; subh.style.marginTop = '8px'; subh.textContent = 'Waitlist:';
      ...
      function acceptApplicantFromActive(jobId, applicantId) {
        ...
        showView('orders');
        activeTab = 'ongoing';
        // keep compatibility with tab id variants
      ...
      // --- Orders rendering (scope-aware if dropdown exists) ---
      function renderGrid() {
        if (!jobGrid) return;
        jobGrid.innerHTML = '';
  
        // Determine scope: newer UI might use jobFilter; older used ordersScope
        const scopeVal = (jobFilter && jobFilter.value) ? jobFilter.value : (document.getElementById('ordersScope') ? document.getElementById('ordersScope').value : 'others');
  
        // if jobFilter exists and has options "others" / "you" then filter accordingly.
        let filtered = [];
        if (jobFilter) {
          const scope = scopeVal;
          if (scope === 'others') {
            filtered = jobs.filter(j => j.postedBy === 'other' && ((activeTab === 'ongoing' && j.status === 'accepted') || (activeTab === 'previous' && j.status === 'completed')));
          } else {
            if (!currentUser) {
              jobGrid.innerHTML = '<div class="empty">No current user set — apply as "Guest" to make "For you" show items.</div>';
              return;
            }
            filtered = jobs.filter(j => j.claimer && j.claimer.name === currentUser && ((activeTab === 'ongoing' && j.status === 'accepted') || (activeTab === 'previous' && j.status === 'completed')));
          }
        } else {
          // no scope dropdown: show accepted/completed across all posts
          filtered = jobs.filter(j => (activeTab === 'ongoing' ? j.status === 'accepted' : (activeTab === 'previous' ? j.status === 'completed' : true)));
        }
        ...
        for (const job of filtered) {
          const tile = document.createElement('div'); tile.className = 'job-tile ' + (activeTab === 'ongoing' ? 'ongoing' : 'previous');
          ...
          if (activeTab === 'ongoing') subtitle.textContent = job.claimer && job.claimer.name ? `${job.claimer.name} is doing this.` : '';
          else subtitle.textContent = job.claimer && job.claimer.name ? `Done by ${job.claimer.name}` : '';
  
          // small Chat badge when showing others (if dropdown exists and set to others)
          ...
          if (activeTab === 'ongoing') {
            const completeBtn = document.createElement('button'); completeBtn.className = 'btn'; completeBtn.type = 'button'; completeBtn.textContent = 'Mark completed';
            completeBtn.onclick = () => markCompleted(job.id);
            actions.appendChild(completeBtn);
          } else {
            const rateBtn = document.createElement('button'); rateBtn.className = 'btn'; rateBtn.type = 'button';
            const already = job.claimer && job.claimer.ratingGiven !== undefined && job.claimer.ratingGiven !== null && job.claimer.ratingGiven !== 'N/A';
            rateBtn.textContent = already ? `Rated: ${job.claimer.ratingGiven}` : 'Rate';
      ...
      // tabs wiring (support multiple ID variants)
      const attachTabHandlers = () => {
        if (ongoingTab) ongoingTab.onclick = () => { activeTab = 'ongoing'; if (ongoingTab.classList) ongoingTab.classList.add('active'); if (previousTab && previousTab.classList) previousTab.classList.remove('active'); renderGrid(); };
        if (previousTab) previousTab.onclick = () => { activeTab = 'previous'; if (previousTab.classList) previousTab.classList.add('active'); if (ongoingTab && ongoingTab.classList) ongoingTab.classList.remove('active'); renderGrid(); };
        if (tabOngoing) tabOngoing.onclick = () => { activeTab = 'ongoing'; if (tabOngoing.classList) tabOngoing.classList.add('active'); if (tabAccepted && tabAccepted.classList) tabAccepted.classList.remove('active'); renderGrid(); };
        if (tabAccepted) tabAccepted.onclick = () => { activeTab = 'previous'; if (tabAccepted.classList) tabAccepted.classList.add('active'); if (tabOngoing && tabOngoing.classList) tabOngoing.classList.remove('active'); renderGrid(); };
      ...
      function markCompleted(jobId) {
        ...
        if (job.claimer.ratingGiven === null || job.claimer.ratingGiven === undefined) job.claimer.ratingGiven = 'N/A';
        saveJobs();
        renderGrid();
      }
      ...
      function rateClaimer(jobId) {
        ...
        if (job.claimer.ratingGiven !== null && job.claimer.ratingGiven !== undefined && job.claimer.ratingGiven !== 'N/A') return alert('Claimer already rated for this job');
  
        const r = prompt(`Rate the claimer "${job.claimer.name}" (1-5):`, '5');
        if (r === null) return;
        const rn = Number(r);
        if (Number.isNaN(rn) || rn < 1 || rn > 5) { alert('Invalid rating — must be 1-5'); return; }
